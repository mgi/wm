...
(defun 1-o-value (val) "Replace nil with one." (if (null val) 1 val))
(defun 0-o-value (val) "Replace nil with zero." (if (null val) 0 val))

(defun size-w/hints (window width height)
  (let ((hints (wm-normal-hints window)))
    (cond (hints (let* ((min-w (1-o-value (wm-size-hints-min-width hints)))
                   ...
                   (let ((new-w (max min-w (+ (* inc-w (truncate width inc-w)) base-w)))
                     ...
                     (values new-w new-h))))
          (t (values (max 1 width) (max 1 height))))))

(defun position-w/hints (window x y)
  (let ((hints (wm-normal-hints window)))
    (cond (hints (let ((inc-w (1-o-value (wm-size-hints-width-inc hints)))
                       (inc-h (1-o-value (wm-size-hints-height-inc hints))))
    ...
    (cond ((sc= *close* state code)
          ...
          (t
           ...
           (when (sc= *resize* state code)
             (let ((hints (wm-normal-hints child))
                   (width (drawable-width child))
